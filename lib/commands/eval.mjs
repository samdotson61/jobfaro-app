...
import { upsertEval, bandConflict, readPipeline, pendingQueue, upsertListingChecks } from '../evaluations.mjs'
...
import { classifyFetchError } from '../liveness.mjs'
...
// A JD-fetch failure can be the same board signal `recheck` listens for: Workday 403s an unposted
// job, Greenhouse/iCIMS 404/410 a closed one. Marking those rows gone here keeps pendingQueue from
// re-serving the same dead postings to every future auto-eval batch. Anything else (network error,
// empty JD) stays an ordinary skip — an unreachable board proves nothing.
// `persist` is an injectable seam for tests (defaults to the real pipeline write).
export function markGoneIfDead(err, row, today, t, say, persist = upsertListingChecks) {
  if (classifyFetchError(err && err.message) !== 'gone') return false
  persist([{ url: row.url, state: 'gone' }], today)
  say('  ' + color.yellow(`✗ ${t('recheck.gone_line', { company: row.company || row.url, role: row.role || '' })}`))
  return true
}
...
async function runAutoEval(flags, positionals, t) {
  ...
  for (const row of targets) {
    ...
    if (!jd) {
      if (!(fetchErr && markGoneIfDead(fetchErr, row, today, t, say))) {
