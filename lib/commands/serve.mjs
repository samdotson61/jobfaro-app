...
export async function runServe(argv = []) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      // Eval feedback (thumbs up/down on a verdict) → the local labeled set `jobfaro calibrate --feedback` reads.
      if (req.method === 'POST' && path === '/eval/feedback') {
        const p = await readBody(req)
        if (!p.url || (p.thumb !== 'up' && p.thumb !== 'down')) return send(res, 400, { ok: false, error: 'url and thumb (up|down) required' }, req)
        appendFeedback({ url: p.url, company: p.company || '', role: p.role || '', score: p.score != null ? p.score : '', band: p.band || '', thumb: p.thumb, date: today() })
